...
fnFs <- "24MA_S64_L001_R1_001.fastq"
#fnRs <- "2_S76_L001_R2_001.fastq"
sample.names <- "24MA_S64"
...
for(i in 1:(length(truncL))){
  for(j in 1:length(learn)){
    for(k in 1:3){
      ...
      param<-rbind(param,list(truncL[i],learn[j],pl))
...
colnames(param)<-c("Tcl","nbases","pool")
...
for (i in 1:nrow(param)){
  ...
  taxa<- assignTaxonomy(seqtab.nochim,"../../../silva_nr_v132_train_set.fa.gz", multithread=T,minBoot =50,outputBootstraps = F, tryRC=F )
  ...
  track <- cbind(paste("track",i,sep=""),out,sum(getUniques(dadaFs)), rowSums(seqtab.nochim),na,length(unique(c(ind_e,ind))),
                 paste("TruncLen_",param$Tcl[[i]],sep=""),paste("nbases_",param$nbases[[i]],sep=""),paste("pool_",param$pool[[i]],sep=""))
  
  colnames(track) <- c("","input","filtered", "denoisedF", "nonchim","assignes","badassignes","Trunclen","nbases","pool")
  ...
  if(i>1){
    write.table(track,paste(sample.names,"track_R1pool.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = F)
  }else{
    write.table(track,paste(sample.names,"track_R1pool.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = T)
  }
}
asv<-c()  
...
write.table(table_taxa[,1:7],paste(sample.names,"table_taxa_R1pool.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = T)
t<-cbind(table_taxa[,1],table_taxa[,9:ncol(table_taxa)])
write.table(t,paste(sample.names,"count_R1pool.csv",sep=""),append=F,sep=",",qmethod="double",row.names = F,col.names = T)
T2<-Sys.time()
