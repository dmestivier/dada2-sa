...
for (i in 1:nrow(param)){
  ...
  if(i>1){
    write.table(track,paste(sample.names,"_error-dada.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = F)
  }else{
    write.table(track,paste(sample.names,"_error-dada.csv",sep=""),append=F,sep=",",qmethod="double",row.names = F,col.names = T)
  }
}
asv<-c()  
...
write.table(table_taxa[,1:7],paste(sample.names,"_error-dada_taxa.csv",sep=""),append=F,sep=",",qmethod="double",row.names = F,col.names = T)
t<-cbind(table_taxa[,1],table_taxa[,9:ncol(table_taxa)])
write.table(t,paste(sample.names,"_error-data_counts.csv",sep=""),append=F,sep=",",qmethod="double",row.names = F,col.names = T)
T2<-Sys.time()
