...
pathDB <- "/home/dmestivier/Travail/ec2m3.paper.dada2/DB"
pathFQ <- "/home/dmestivier/Travail/ec2m3.paper.dada2/Data"
pathRES<- "/work/dmestivier/Travail/ec2m3.paper.dada2/PDF/Error"

##NOM FICHIER 
T1<-Sys.time()

fnFs <- paste( pathFQ, "/19KA_S72_L001_R1_001.fastq", sep="" )
fnRs <- paste( pathFQ, "/19KA_S72_L001_R2_001.fastq", sep="" )
sample.names <- "19KA_S72"

param<-NULL

learn<-c(1e6,1e8,1e10,1e12)
#pool<-c(T,F)
truncL<-c(c(0,0),c(250,230),c(180,160),c(250,200))
MAX_CONSIST=c(5,10,20)
pl<-0

for(i in 1:(length(truncL)/2)){
  for(j in 1:length(learn)){
		for( l in 1:length(MAX_CONSIST) ){
      param<-rbind(param,list(c(truncL[2*(i-1)+1],truncL[2*(i-1)+2]),learn[j], MAX_CONSIST[l]))
		} 
    }
}

param<-as.data.frame(param)
colnames(param)<-c("Tcl","nbases", "MaxConsist" )
...
for (i in 1:nrow(param)){
  ####pipeline dada ########################################
  filtFs <- file.path(pathFQ, "filtered2", paste0(sample.names, "_F_filt.fastq.gz"))
  filtRs <- file.path(pathFQ, "filtered2", paste0(sample.names, "_R_filt.fastq.gz"))
  out<-filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=param$Tcl[[i]],maxN=0, maxEE=c(2,2), 
                     truncQ=2, rm.phix=T,compress=TRUE, multithread = T,trimLeft =0)
  errF <- learnErrors(filtFs, multithread=TRUE,nbases=param$nbases[[i]])
  errR <- learnErrors(filtRs, multithread=TRUE,nbases=param$nbases[[i]])
  plotErrors(errF, nominalQ=TRUE)
  dadaFs <- dada(filtFs, err=errF, multithread=TRUE,pool=param$pool[[i]])
  dadaRs <- dada(filtRs, err=errR, multithread=TRUE,pool=param$pool[[i]])
  ...
  track <- cbind(paste("track",i,sep=""),out,sum(getUniques(dadaFs)),sum(getUniques(dadaRs)),sum(getUniques(mergers)), rowSums(seqtab.nochim),na,length(unique(c(ind_e,ind))),
                 paste("TruncLen_",paste(param$Tcl[[i]][1],param$Tcl[[i]][2],sep="-"),sep=""),paste("nbases_",param$nbases[[i]],sep=""),paste("pool_",param$pool[[i]],sep=""))
  
  colnames(track) <- c("","input","filtered", "denoisedF", "denoisedR", "merged", "nonchim","assignes","badassignes","Trunclen","nbases","pool")
  ...
  if(i>1){
    write.table(track,paste(sample.names,"_error.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = F)
  }else{
    write.table(track,paste(sample.names,"_error.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = T)
  }
}
asv<-c()  
...
write.table(table_taxa[,1:7],paste(sample.names,"_taxa.csv",sep=""),append=T,sep=",",qmethod="double",row.names = F,col.names = T)
