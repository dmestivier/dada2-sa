...
pathFQ <- paste( pathDM, "/ec2m3.paper.dada2/Data-CCR1", sep="" )
pathRES<- paste( pathDM, "/ec2m3.paper.dada2/CCR1/PDS/FilterTrim/Trunclen", sep="" )
...
fnFs <- paste( pathFQ, "/", sample.name, "-1_AMPLICON.fastq", sep="" )
fnRs <- paste( pathFQ, "/", sample.name, "-2_AMPLICON.fastq", sep="" )
...
for (i in 1:nrow(param)){
  ...
  ###
  ### dm <2021-12-14 mar.> 
  ### Export ASV as FASTA. One file per parameter sets
  ###

  myOutfn = paste( sample.name, "-track", i, ".fasta", sep="" )
  myFasta = c()

  for( iasv in seq(from=1, to=length(rownames(taxa)),by=1) )
  {
	  headFasta = paste( ">", sample.name, "_asv-", iasv, "_track", i, sep="" )
	  myFasta   = rbind( myFasta, headFasta )
	  myFasta   = rbind( myFasta, rownames(taxa)[iasv] )
  }

  write.table( myFasta, myOutfn, sep="\t", row.names=F, col.names=F, quote=F )
